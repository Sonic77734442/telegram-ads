            ...
            <div className="flex gap-3">
              {[1, 2, 3, 4].map((n) => (
                <button
                  ...
                  className={`h-[32px] w-[75.3px] rounded-[6px] border text-[13px] font-medium transition-all duration-150 ${
...
const ChannelTargetingSummary = ({
  ...
  const hasTarget = langs.length || topics.length || targetChannels.length;
  const hasExclude = exTopics.length || excludeChannels.length;
      ...
      {Boolean(hasTarget) && (
        <div className="flex gap-3">
          ...
          <p>
            ...
            {langs.length > 0 && (
              <>
                {" "}for channels whose subscribers speak <BoldList items={langs} />
              </>
            )}
            {topics.length > 0 && (
              <>
                {" "}and are related to <BoldList items={topics} />
              </>
            )}
            {targetChannels.length > 0 && (
              <>
                {" "}and specific channels <BoldList items={targetChannels} />
              </>
            )}
          </p>
        </div>
      ...
      {Boolean(hasExclude) && (
        <div className="flex gap-3">
          ...
          <p>
            ...
            {exTopics.length > 0 && (
              <>
                {" "}in channels related to <BoldList items={exTopics} />
              </>
            )}
            {excludeChannels.length > 0 && (
              <>
                {" "}or in specific channels <BoldList items={excludeChannels} />
              </>
            )}
          </p>
        </div>
...
const Input = (props: React.InputHTMLAttributes<HTMLInputElement>) => (
  <input {...props} className={`min-h-[40px] w-full rounded-[4px] border border-[#d9d9d9] bg-white px-3 py-[5px] focus:border-blue-500 focus:ring-1 focus:ring-blue-500 ${props.className || ""}`} />
);

const Textarea = (props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => (
  <textarea {...props} className="min-h-[40px] w-full resize-none rounded-[4px] border border-[#d9d9d9] bg-white px-3 py-[5px] focus:border-blue-500 focus:ring-1 focus:ring-blue-500" />
);
